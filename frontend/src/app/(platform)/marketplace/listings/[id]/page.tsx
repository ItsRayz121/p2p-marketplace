...
export default function AdListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleConfirmBidDetails = async () => {
    ...
    if (ad?.side === 'sell' && confirmDeliveryMethod && confirmDeliveryAddress.trim()) {
      const label = confirmDeliveryMethod === 'wallet_blockchain' ? (ad.network ?? '') : confirmDeliveryMethod
      const r = validateAddressForNetwork(confirmDeliveryAddress.trim(), label)
      if (!r.valid) { setConfirmError(r.reason ?? 'Invalid receiving address'); return }
    }
    setConfirmError('')
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-5">
                ...
                {instantDeliveryMethod && (() => {
                  ...
                  return (
                    <div className="mt-2 space-y-2">
                      ...
                      />
                      {/* On-chain wallet → validate format. Exchange/internal transfer →
                          no format enforcement (justified by the transfer screenshot). */}
                      {isWallet ? (() => {
                        const t = instantDeliveryAddress.trim()
                        if (!t || !effNet) return null
                        const r = validateAddressForNetwork(t, effNet)
                        return r.valid
                          ? <p className="text-xs text-green-600 dark:text-green-400 flex items-center gap-1"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>Valid {effNet} address</p>
                          : <p className="text-xs text-danger">{r.reason}</p>
                      })() : (
                        <p className="text-xs text-text-muted">The seller sends to this {instantDeliveryMethod} account and attaches a transfer screenshot as proof.</p>
                      )}
                    </div>
                  )
                })()}
      ...
      {/* ─── Confirm Bid Details Modal ───────────────────────────────────── */}
      {/* Once the confirm window lapses (even while this modal is open) the whole
          form is sealed and replaced by a dead-end "expired" state — no payment
          details can be submitted against an expired bid. */}
      {showConfirmModal && myActiveBid && (
        new Date(myActiveBid.expiresAt).getTime() <= nowTs ? (
          <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
            <div className="bg-surface rounded-2xl w-full max-w-md p-6 space-y-4 border border-border shadow-card-lg">
              <div>
              ...
              <button onClick={() => setShowConfirmModal(false)} className="w-full bg-primary text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-primary/90 transition-colors">
                Close
              </button>
            </div>
          </div>
                ...
                {confirmDeliveryMethod && (
                  <div className="mt-2">
                    ...
                    <input type="text"
                      ...
                      className="w-full border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                    />
                    {confirmDeliveryMethod === 'wallet_blockchain' ? (() => {
                      const label = ad.network ?? ''
                      const t = confirmDeliveryAddress.trim()
                      if (!t || !label) return null
                      const r = validateAddressForNetwork(t, label)
                      return r.valid
                        ? <p className="text-xs text-green-600 dark:text-green-400 mt-1 flex items-center gap-1"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>Valid {ad.network} address</p>
                        : <p className="text-xs text-danger mt-1">{r.reason}</p>
                    })() : (
                      <p className="text-xs text-text-muted mt-1">The seller sends to this {confirmDeliveryMethod} account and attaches a transfer screenshot as proof.</p>
                    )}
                  </div>
                )}
              </div>
