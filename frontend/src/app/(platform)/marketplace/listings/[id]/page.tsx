...
interface SavedPaymentMethod {
  id: string; type: string; displayName: string; accountName: string
...
export default function AdListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [myActiveBid, setMyActiveBid] = useState<AdActivity['myBid']>(null)
  ...
  const fetchActivity = useCallback(async () => {
    try {
      ...
      setActivity(res)
      if (res.myBid !== undefined) setMyActiveBid(res.myBid)
    } catch { /* ignore */ }
  }, [id])
  ...
  // Load payment methods once user is known
  const [methodsLoaded, setMethodsLoaded] = useState(false)
  if (user && !methodsLoaded) {
    setMethodsLoaded(true)
    fetchMyPaymentMethods().then(setMyMethods)
  }
  ...
  // SELL ad: lister is SELLER, taker is BUYER → buyer picks seller's payment account to pay to
  // BUY ad: lister is BUYER, taker is SELLER → seller picks own receiving account
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-5">
      ...
      <div className="bg-white border border-border rounded-xl p-6">
        <div className="flex items-start justify-between gap-4 mb-5">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-xl font-bold text-primary">$</div>
            <div>
              <h1 className="text-xl font-bold text-text-primary">
                {isMine
                  ? (isSellAd ? 'Sell' : 'Buy')
                  : (isSellAd ? 'Buy' : 'Sell')} USDT
              </h1>
              <p className="text-text-muted text-sm">{ad.network} · {ad.coin}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 border-t border-border pt-4">
          ...
          <div>
            ...
            <p className="font-bold text-text-primary">{Number(ad.availableAmount).toFixed(4)} USDT</p>
          </div>
          <div>
            <p className="text-xs text-text-muted">Min / Max</p>
            <p className="font-bold text-text-primary">{Number(ad.minOrder).toLocaleString()} – {Number(ad.maxOrder).toLocaleString()} USDT</p>
          </div>
          <div>
      ...
      <div className="bg-white border border-border rounded-xl p-5">
        <h2 className="font-semibold text-text-primary mb-3">{isSellAd ? 'Seller' : 'Buyer'}</h2>
          ...
          <div>
            ...
            {ad.user.tradeStats && (
              <p className="text-xs text-text-muted">
                {ad.user.tradeStats.completedTrades} completed of {ad.user.tradeStats.totalTrades} trades ·{' '}
                {(Number(ad.user.tradeStats.completionRate) * 100).toFixed(0)}% completion
              </p>
            )}
          </div>
        </div>
      ...
      {/* Network info */}
      <div className="bg-white border border-border rounded-xl p-5">
        <h2 className="font-semibold text-text-primary mb-2">USDT Network</h2>
        <p className="text-sm text-text-muted">
          This listing operates on <span className="font-semibold text-text-primary">{ad.network}</span>.
          {isSellAd && !isMine && ' Make sure to provide a receiving address on this network.'}
        </p>
      </div>
                  ...
                  {activity.bids.items.map((bid) => (
                    <div key={bid.id} className="flex items-start justify-between gap-3 bg-surface rounded-xl px-4 py-3">
                      <div className="min-w-0">
                        <p className="font-semibold text-text-primary">PKR {Number(bid.pricePerUnit).toLocaleString()} / USDT</p>
                        <p className="text-xs text-text-muted mt-0.5">
                          {bid.bidder?.username} · {Number(bid.usdtAmount).toLocaleString()} USDT · PKR {Number(bid.fiatAmount).toLocaleString()} total
                        </p>
                    ...
                    <div key={t.orderRef} className="flex items-start justify-between gap-3 bg-surface rounded-xl px-4 py-3">
                      <div className="min-w-0">
                        <p className="font-semibold text-text-primary">
                          {Number(t.amount).toLocaleString()} USDT · PKR {Number(t.fiatAmount).toLocaleString()}
                        </p>
                        <p className="text-xs text-text-muted mt-0.5">
                          {t.buyer.username} → {t.seller.username} · PKR {Number(t.price).toLocaleString()}/USDT
                        </p>
                      </div>
      ...
      {/* CTA — only for non-owners on active listing with no active bid */}
      {!isMine && ad.status === 'active' && !myActiveBid && (
        ...
        >
          {isSellAd ? 'Buy USDT — Place Bid' : 'Sell USDT — Place Bid'}
        </button>
      )}
      ...
      {showBidModal && (
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1.5">Your bid price per USDT (PKR)</label>
            ...
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1.5">
                {isSellAd ? 'How much USDT do you want to buy?' : 'How much USDT will you sell?'}
              </label>
              <div className="relative">
                <input type="number" min={Number(ad.minOrder)} max={Math.min(Number(ad.maxOrder), Number(ad.availableAmount))} step="0.01"
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted text-sm">USDT</span>
              </div>
              <p className="text-xs text-text-muted mt-1">Min {Number(ad.minOrder).toLocaleString()} · Max {Number(ad.maxOrder).toLocaleString()} USDT</p>
            </div>
            ...
            {bidPrice && bidAmount && (() => {
              ...
              return (
                <div className="bg-surface rounded-xl border border-border p-4 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-text-muted">Your bid price</span><span className="font-medium">PKR {parseFloat(bidPrice).toLocaleString()}/USDT</span></div>
                  <div className="flex justify-between"><span className="text-text-muted">USDT amount</span><span className="font-medium">{parseFloat(bidAmount).toLocaleString()} USDT</span></div>
                  <div className="flex justify-between border-t border-border pt-2 font-semibold"><span className="text-text-muted">{isSellAd ? 'Total payable' : 'Total you receive'}</span><span>PKR {total.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                </div>
              )
            })()}

            <p className="text-xs text-text-muted bg-blue-50 border border-blue-100 rounded-xl px-3 py-2">
              Payment details and your USDT address will be collected after the owner accepts your bid.
            </p>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-surface rounded-xl border border-border p-4 space-y-1 text-sm">
              <div className="flex justify-between"><span className="text-text-muted">Bid price</span><span className="font-medium">PKR {Number(myActiveBid.pricePerUnit).toLocaleString()}/USDT</span></div>
              <div className="flex justify-between"><span className="text-text-muted">Amount</span><span className="font-medium">{Number(myActiveBid.usdtAmount).toLocaleString()} USDT</span></div>
              <div className="flex justify-between border-t border-border pt-1 font-semibold"><span className="text-text-muted">{isSellAd ? 'Total payable' : 'Total you receive'}</span><span>PKR {Number(myActiveBid.fiatAmount).toLocaleString()}</span></div>
            </div>
            ...
            {isSellAd && (
              <div>
                <label className="block text-sm font-medium text-text-primary mb-1.5">
                  Your USDT receiving address ({ad.network})
                </label>
                <input type="text"
                ...
                />
                <p className="mt-1 text-xs text-text-muted">The seller will send USDT here after your payment is confirmed.</p>
              </div>
            )}
