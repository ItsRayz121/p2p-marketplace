  ...
  return (
    <Modal isOpen onClose={onClose} title="Sweep Platform Fees to Treasury">
      <div className="space-y-4 text-sm">
        {/* Warning */}
        <div className="bg-amber-950/50 border border-amber-700/50 rounded p-3 text-amber-300 text-xs space-y-1">
        ...
        <div className="bg-gray-800 rounded p-3 space-y-2 text-xs font-mono">
          <div className="flex justify-between">
            ...
            <span className="text-emerald-400">{row.token}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-400">Network</span>
            <span className="text-blue-400">{CHAIN_LABEL[row.chain] ?? row.chain}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-400">Available to sweep</span>
            <span className="text-white">{fmtAmount(row.available)} {row.token}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-400">Destination (treasury)</span>
            <span className="text-gray-300 break-all">{treasuryAddress ?? '—'}</span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="radio" checked={mode === 'all'} onChange={() => setMode('all')} className="accent-emerald-500" />
            <span>Sweep all available — <strong>{fmtAmount(row.available)} {row.token}</strong></span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="radio" checked={mode === 'custom'} onChange={() => setMode('custom')} className="accent-emerald-500" />
          ...
          {mode === 'custom' && (
            <div className="ml-6 flex items-center gap-2">
              <input
                ...
                className="bg-gray-800 border border-gray-600 rounded px-2 py-1 text-white w-36 text-sm"
              />
              <span className="text-gray-400 text-xs">{row.token} (max {fmtAmount(row.available)})</span>
            </div>
          )}
        </div>
...
export default function PlatformRevenuePage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      {/* Sweep result banner */}
      {sweepResult && (
        <div className="bg-emerald-950/50 border border-emerald-600/50 rounded-lg p-4 text-sm">
          <p className="font-semibold text-emerald-300 mb-2">Sweep successful!</p>
          <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-xs font-mono text-gray-300">
            <span className="text-gray-500">Amount swept</span>
            ...
            <span>
              {txUrl(sweepResult.chain, sweepResult.txHash) ? (
                <a href={txUrl(sweepResult.chain, sweepResult.txHash)!} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">
          ...
          <button onClick={() => setSweepResult(null)} className="mt-3 text-xs text-gray-500 hover:text-white">Dismiss</button>
        </div>
      )}

      {/* Sweep error banner */}
      {sweepError && (
        <div className="bg-red-950/50 border border-red-700/50 rounded-lg p-4 text-sm text-red-300">
          <p className="font-semibold mb-1">Sweep failed</p>
          <p className="text-xs">{sweepError}</p>
          <button onClick={() => setSweepError(null)} className="mt-2 text-xs text-gray-500 hover:text-white">Dismiss</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          badge={s.allTime.available > 0 ? { text: 'Ready', color: 'bg-emerald-900 text-emerald-300' } : { text: 'None', color: 'bg-gray-800 text-gray-400' }}
        />
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-lg p-4 text-xs font-mono">
        <p className="text-gray-400 text-xs uppercase tracking-wide mb-2 font-sans font-semibold">Treasury Wallet Addresses</p>
        <div className="space-y-1">
          <div className="flex gap-4"><span className="text-gray-500 w-24">EVM</span><span className="text-white break-all">{s.treasuryAddresses.evm ?? 'Not configured'}</span></div>
          <div className="flex gap-4"><span className="text-gray-500 w-24">TRON</span><span className="text-white break-all">{s.treasuryAddresses.tron ?? 'Not configured'}</span></div>
        </div>
        <p className="text-gray-600 text-xs mt-2 font-sans">Sweep destinations are derived from your configured mnemonic (HD index 101 for EVM, 100 for TRON).</p>
      </div>

      {/* Sweepable breakdown */}
      <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
        <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wide mb-3">Available to Sweep — by Token &amp; Network</h2>
        {s.sweepable.length === 0 ? (
          <p className="text-sm text-gray-500">No platform fees collected yet.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-500 border-b border-gray-700">
                  ...
                  <th className="text-right py-2 text-emerald-400">Available</th>
                  <th className="text-right py-2">Action</th>
                </tr>
              </thead>
              <tbody>
                {s.sweepable.map(row => (
                  <tr key={`${row.token}:${row.chain}`} className="border-b border-gray-800 hover:bg-gray-800/40">
                    <td className="py-2 font-mono text-emerald-400">{row.token}</td>
                    <td className="py-2 text-blue-400">{CHAIN_LABEL[row.chain] ?? row.chain}</td>
                    <td className="py-2 text-right text-gray-300 font-mono">{fmtAmount(row.collected)}</td>
                    <td className="py-2 text-right text-gray-500 font-mono">{fmtAmount(row.swept)}</td>
                    <td className="py-2 text-right font-mono font-semibold text-white">{fmtAmount(row.available)}</td>
                    <td className="py-2 text-right">
                      {row.canSweep ? (
                        ...
                        <span className="text-xs text-amber-500">Manual (TRC20)</span>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      {/* Sweep history */}
      <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
        <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wide mb-3">Sweep History</h2>
        {sweepHistory.length === 0 ? (
          <p className="text-sm text-gray-500">No sweeps yet.</p>
        ) : (
          <>
            <div className="overflow-x-auto">
                  ...
                  {sweepHistory.map(e => {
                    ...
                    return (
                      <tr key={e.id} className="border-b border-gray-800 hover:bg-gray-800/40">
                        ...
                        <td className="py-2 text-blue-400">{CHAIN_LABEL[e.chain] ?? e.chain}</td>
                        <td className="py-2 text-right font-mono text-emerald-400">
                          {fmtAmount(e.tokenAmount)} {e.tokenSymbol}
                        </td>
                        <td className="py-2 pl-4">
                          {url ? (
                            <a href={url} target="_blank" rel="noopener noreferrer" className="font-mono text-blue-400 hover:underline">
                              {e.txHash?.slice(0, 12)}…
                            </a>
                          ) : <span className="text-gray-600 font-mono">{e.txHash?.slice(0, 12)}…</span>}
                        </td>
                        <td className="py-2 pl-4 font-mono text-gray-400 max-w-xs truncate">{e.toAddress ?? '—'}</td>
                      </tr>
                    )
                  })}
                </tbody>
            ...
            {sweepPagination.pages > 1 && (
              <div className="flex items-center justify-between mt-3 text-xs text-gray-400">
                ...
                <div className="flex gap-2">
                  <button disabled={sweepPage <= 1} onClick={() => setSweepPage(p => p - 1)} className="px-2 py-1 border border-gray-600 rounded disabled:opacity-30">Prev</button>
                  <span>{sweepPage}/{sweepPagination.pages}</span>
                  <button disabled={sweepPage >= sweepPagination.pages} onClick={() => setSweepPage(p => p + 1)} className="px-2 py-1 border border-gray-600 rounded disabled:opacity-30">Next</button>
                </div>
              </div>
            )}
          </>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-xs text-gray-500 border-b border-gray-700"><th className="text-left py-1.5">Token</th><th className="text-right py-1.5">Collected</th><th className="text-right py-1.5">USD</th><th className="text-right py-1.5">Count</th></tr></thead>
            <tbody>
              {s.byToken.map(r => (
              ...
              {s.byToken.length === 0 && <tr><td colSpan={4} className="py-4 text-center text-gray-600">No data</td></tr>}
            </tbody>
          </table>
        </div>
        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-xs text-gray-500 border-b border-gray-700"><th className="text-left py-1.5">Network</th><th className="text-right py-1.5">Collected</th><th className="text-right py-1.5">USD</th><th className="text-right py-1.5">Count</th></tr></thead>
            <tbody>
              {s.byChain.map(r => (
                <tr key={r.chain} className="border-b border-gray-800 hover:bg-gray-800/40">
                  <td className="py-2 font-mono text-blue-400">{CHAIN_LABEL[r.chain] ?? r.chain}</td>
                  <td className="py-2 text-right text-white">{fmtAmount(r.amount)}</td>
                  <td className="py-2 text-right text-gray-400">{fmtUsd(r.usdAmount)}</td>
                  <td className="py-2 text-right text-gray-500">{r.count}</td>
                </tr>
              ))}
              {s.byChain.length === 0 && <tr><td colSpan={4} className="py-4 text-center text-gray-600">No data</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wide">Fee Collection History</h2>
          <div className="flex flex-wrap gap-2 text-xs">
            <input type="text" placeholder="Token" value={feeToken} onChange={e => { setFeeToken(e.target.value.toUpperCase()); setFeePage(1) }} className="bg-gray-800 border border-gray-600 rounded px-2 py-1 text-white placeholder-gray-500 w-24" />
            <input type="text" placeholder="Network" value={feeChain} onChange={e => { setFeeChain(e.target.value.toUpperCase()); setFeePage(1) }} className="bg-gray-800 border border-gray-600 rounded px-2 py-1 text-white placeholder-gray-500 w-28" />
            <input type="date" value={feeFrom} onChange={e => { setFeeFrom(e.target.value); setFeePage(1) }} className="bg-gray-800 border border-gray-600 rounded px-2 py-1 text-white w-34" />
            <span className="text-gray-500 self-center">→</span>
            <input type="date" value={feeTo} onChange={e => { setFeeTo(e.target.value); setFeePage(1) }} className="bg-gray-800 border border-gray-600 rounded px-2 py-1 text-white w-34" />
            <input type="text" placeholder="Search TX" value={feeSearch} onChange={e => { setFeeSearch(e.target.value); setFeePage(1) }} className="bg-gray-800 border border-gray-600 rounded px-2 py-1 text-white placeholder-gray-500 w-36" />
            {(feeToken || feeChain || feeFrom || feeTo || feeSearch) && (
              <button onClick={() => { setFeeToken(''); setFeeChain(''); setFeeFrom(''); setFeeTo(''); setFeeSearch(''); setFeePage(1) }} className="text-gray-400 hover:text-white border border-gray-600 rounded px-2 py-1">Clear</button>
            )}
          </div>
        </div>
        {feeEntries.length === 0 ? (
          <p className="text-gray-500 text-sm py-4">No fee entries found.</p>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                  ...
                  {feeEntries.map(e => {
                    ...
                    return (
                      <tr key={e.id} className="border-b border-gray-800 hover:bg-gray-800/40">
                        <td className="py-2 text-gray-400 whitespace-nowrap">{fmtDate(e.createdAt)}</td>
                        <td className="py-2 text-blue-400">{CHAIN_LABEL[e.chain] ?? e.chain}</td>
                        <td className="py-2 text-right font-mono text-emerald-400">{fmtAmount(e.tokenAmount)} {e.tokenSymbol}</td>
                        <td className="py-2 text-right text-gray-400">{fmtUsd(e.usdAmount)}</td>
                        <td className="py-2 pl-3">
                          {url ? <a href={url} target="_blank" rel="noopener noreferrer" className="font-mono text-blue-400 hover:underline">{e.txHash?.slice(0, 10)}…</a> : <span className="font-mono text-gray-500">{e.txHash?.slice(0, 10) ?? '—'}</span>}
                        </td>
                        <td className="py-2 pl-3">
                          {wdId ? <a href={`/admin/withdrawals?search=${wdId}`} className="font-mono text-gray-400 hover:text-white">{wdId.slice(-8)}</a> : <span className="text-gray-600">—</span>}
                        </td>
                      </tr>
                    )
                  })}
            ...
            {feePagination.pages > 1 && (
              <div className="flex items-center justify-between mt-3 text-xs text-gray-400">
                ...
                <div className="flex gap-2">
                  <button disabled={feePage <= 1} onClick={() => setFeePage(p => p - 1)} className="px-2 py-1 border border-gray-600 rounded disabled:opacity-30">Prev</button>
                  <span>{feePage}/{feePagination.pages}</span>
                  <button disabled={feePage >= feePagination.pages} onClick={() => setFeePage(p => p + 1)} className="px-2 py-1 border border-gray-600 rounded disabled:opacity-30">Next</button>
                </div>
              </div>
            )}
          </>
