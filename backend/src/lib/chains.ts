...
export interface ChainConfig {
  ...
  tokens: TokenConfig[]
  /** Block explorer base URL — `${explorerBase}/tx/<hash>` and `/address/<addr>` */
  explorerBase: string
}
...
export function explorerTxUrl(chainId: string, txHash: string): string | undefined {
  ...
  return c ? `${c.explorerBase}/tx/${txHash}` : undefined
}

export function explorerAddressUrl(chainId: string, address: string): string | undefined {
  const c = getChainById(chainId)
  return c ? `${c.explorerBase}/address/${address}` : undefined
}
