...
- ✅ **D2. Tables cut off / columns half-visible (raw horizontal clipping).** (Phase 7 audit + Phase 9 cards) Audited all 29 admin tables — all wrapped in `overflow-x-auto`. Then, per user request, upgraded the primary wide tables to a proper mobile **stacked-card** layout (NEW `.stack-sm` CSS util + `data-label` per `<td>`): on phones the header hides and each row becomes a labeled card so EVERY column is visible with NO horizontal scroll. Converted: Referrals (Referred Users + Top Inviters), Disputes, Trade Ratings, KYC queue, Merchant-KYC, Trades, Users, Withdrawals. Desktop tables untouched (cards only <640px). `.stack-hide` opts a decorative cell out on mobile. Affected: Referrals (Inviter/KYC/Referred By columns clipped), Trade Ratings, KYC queue, etc. Fix: proper responsive table treatment on mobile (horizontal scroll with visible affordance, or stacked card rows) so all columns are reachable without content being silently chopped.
